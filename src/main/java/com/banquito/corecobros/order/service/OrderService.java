...
public class OrderService {
    ...
    public void createOrderAutomaticDebit(MultipartFile file, OrderDTO dto) {
        ...
        while (orderRepository.existsByUniqueId(uniqueId)) {
            uniqueId = generateUniqueId();
            log.info("Generado nuevo uniqueId: {}", uniqueId);
        }
        order.setUniqueId(uniqueId);
        ...
        try {
            log.info("Procesando el archivo CSV para la orden con ID: {}", savedOrder.getOrderId());
            ...
            log.info("Total amount calculado: {}", totalAmount);
    
            savedOrder.setTotalAmount(totalAmount);
            this.orderRepository.save(savedOrder);
            log.info("Orden actualizada con el monto total: {}", savedOrder);
        } catch (Exception e) {
    ...
    public OrderDTO updateOrderStatus(String uniqueId, String status) {
        try {
            if (status == null || status.isEmpty()) {
                log.error("El estado proporcionado es nulo o vacío");
            ...
            if (order == null) {
                log.error("No se encontró una orden con el ID único: {}", uniqueId);
                throw new NoSuchElementException("No se encontró la orden con el ID único proporcionado");
            }
            
            log.info("Se va a cambiar el status a la orden con ID único: {}", uniqueId);
            order.setStatus(status);
            orderRepository.save(order);
            
            return this.orderMapper.toDTO(order);
        } catch (Exception e) {
            log.error("Error inesperado al actualizar el estado de la orden: {}", e.getMessage(), e);
            // Manejo adicional del error, si es necesario
            throw new RuntimeException("Error inesperado al actualizar el estado de la orden", e);
        }
    }
    ...
    public List<OrderDTO> getOrdersByStatusAndServiceId(String serviceId) {
        List<Order> orders = orderRepository
                        .findByServiceIdAndStatus(serviceId, "APR");
        return orders.stream()
                .map(orderMapper::toDTO)
                .collect(Collectors.toList());
    }
    ...
    public void processAutomaticDebits() {
        ...
        List<OrderDTO> orders = this.getOrdersByStatusAndServiceId("JXM0025321");
